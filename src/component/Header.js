    ...
    <div className="min-h-full">
      <nav className="bg-gray-800">
              ...
              <div className="hidden md:block">
                <div className="ml-10 flex items-baseline space-x-4">
                  {MenuItems.map((item, index)=>{
                    return(
                      <ul>
                        <li key={index} className={item.cName} onClick={()=>handleLocalPageChange(`${item.url}`)} aria-current={item.url === LocalPage ? 'page' : 'none'}>{item.Title}</li>

                      </ul>
                    );
                  })}
                  
                  {/* <a href="/public/index.html" className="rounded-md bg-gray-900 px-3 py-2 text-sm font-medium text-white" aria-current="page">Accueil</a>
                  <a href="/public/index.html?action=parcours" className="rounded-md px-3 py-2 text-sm font-medium text-gray-300 hover:bg-gray-700 hover:text-white">Mon parcours</a>
                  <a href="/public/index.html?action=projet" className="rounded-md px-3 py-2 text-sm font-medium text-gray-300 hover:bg-gray-700 hover:text-white">Projet</a>
                  <a href="/public/index.html?action=calendrier" className="rounded-md px-3 py-2 text-sm font-medium text-gray-300 hover:bg-gray-700 hover:text-white">Calendrier</a>
                  <a href="/public/index.html?action=contact" className="rounded-md px-3 py-2 text-sm font-medium text-gray-300 hover:bg-gray-700 hover:text-white">Contact</a>*/}
                </div>
              </div>
            </div>
            <div className="hidden md:block">
        ...
        <div className="md:hidden" id="mobile-menu">
          <div className={`'space-y-1 px-2 pb-3 pt-2 sm:px-3' ${isMenuOpen ? 'block' : 'hidden' }`}>
            {MenuItems.map((item, index)=>{
              return(
